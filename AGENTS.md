...
- `config/omp/agent/config.yml` is copied to `~/.omp/agent/config.yml` — omp's persistent settings (`modelRoles`, theme, etc.). A `setupVersion: 2` marker (omp's current `CURRENT_SETUP_VERSION`) plus a pinned `composer.shape` is included so the image ships "already set up" and skips the first-run wizard, which would otherwise ask about rounded border / composer shape and other settings on every startup.
  ...
  - Sources: `mattpocock/skills` (all non-deprecated categories), `cursor/plugins` `pstack/skills`, `addyosmani/agent-skills`, and `coreyhaines31/marketingskills` (only `schema` and `programmatic-seo`); name collisions (`tdd`, `teach`) keep the mattpocock copy canonical and prefix later sources as `<source>-<name>` (`pstack-`, `addyosmani-`, `marketing-`). Rebuild the image to refresh from upstream (shallow clones at `main`); override repo/ref with `MATT_POCOCK_REPO`/`MATT_POCOCK_REF`/`PSTACK_REPO`/`PSTACK_REF`/`ADDYOSMANI_REPO`/`ADDYOSMANI_REF`/`MARKETING_REPO`/`MARKETING_REF` build-args.
